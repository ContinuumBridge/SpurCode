...
|  9     | XV    | App value. See below 0xFF = ignore |
|  A     | XS    | App state. See below |
...
**App Value and App State**
The app_value is sent to the node in a config message. When an app_value message is received:

    If XV of the current state == app_value
        If XS of the current state == 0xFF
            Set the current display to display number XV
        else
            Set the current state to state number XS
            
In this way, we can either stay in the same state and change what is displayed or go to a completely different state.
